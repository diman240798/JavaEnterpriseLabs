...
public class ReceiptCrudCliProcessor {
    ...
    public void processReceiptCrudApi(String[] args, List<String> crudActionsSupportedList) {
        ...
        BaseDataProvider<Receipt> dataProviderImpl = RepositoriesUtil.getReceiptsDataProvider(dataProviderStr, repositories);
        ...
        if (method.equals(Constants.GET)) {
            if (checkActionSupported(method, crudActionsSupportedList)) {
                ...
                Receipt item = dataProviderImpl.getById(id).get();
                LOG.info(item.toString());
            }
        } else if (method.equals(Constants.GET_ALL)) {
            if (checkActionSupported(method, crudActionsSupportedList)) {
                List<Receipt> all = dataProviderImpl.getAll();
                for (Receipt item : all) {
                    LOG.info(item.toString());
                }
            }
        } else if (method.equals(Constants.INSERT)) {
            if (checkActionSupported(method, crudActionsSupportedList)) {
                Receipt category = getModel(modelData.get());
